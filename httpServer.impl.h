...
void handleCmd(){
    switch((serverCmd)_port80.arg("cmd").toInt())
    {
        ...
        case GET_DATA: {
            //_port80.send(200, "application/json", "[\"" +  + "\"]");
            _port80.send(200, "application/json", "[\"" + DB.exportData(_port80.arg("page").toInt()) + "\"]");
            break;
        }
        ...
        case UPDATE_FROM_FILE:{
            getStringFile.setupUpdate(_port80.arg("filePath"));
            break;
        }
        ...
        case RETURN_ARGUMENTS_TO_JSON:{ String c = String();
...
void serverBegin(){
	...
	_port80.on("/UpFile", HTTP_GET, [](){_port80.send(200,"text/html", "<html><body><form method='POST' action='' enctype='multipart/form-data'><input type='file' name='update' multiple><input type='submit' value='Update'></form></body></html>");});
    _port80.on("/UpFile", HTTP_POST,[](){_port80.send(200, "text/plain", "ok"); },[](){handleUpFile(_port80.upload());});
	_port80.on("/inline", [](){_port80.send(200,"text/plain", "hello");});
